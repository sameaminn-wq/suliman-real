...
const menuItems = [
  { 
    ...
    path: '/dashboard', // المسار الصحيح
    access: ['ADMIN', 'SECRETARY', 'EMPLOYEE'] 
  },
  { 
    name: 'إضافة وحدة', 
    icon: PlusCircle, 
    path: '/dashboard/add-property', // تم التعديل ليطابق مجلدك الفعلي
    access: ['ADMIN', 'SECRETARY'] 
  },
  { 
    name: 'إدارة العقارات', 
    icon: Building2, 
    path: '/dashboard/properties', // تأكد من إنشاء هذا المجلد
    access: ['ADMIN', 'SECRETARY', 'EMPLOYEE'] 
  },
  { 
    name: 'العملاء (Leads)', 
    icon: Users, 
    path: '/dashboard/customers', // يطابق مجلدك الفعلي
    access: ['ADMIN', 'SECRETARY', 'EMPLOYEE'] 
  },
  { 
    name: 'الرسائل', 
    icon: Mail, 
    path: '/dashboard/messages', // تأكد من إنشاء هذا المجلد
    access: ['ADMIN', 'SECRETARY'] 
  },
  { 
    name: 'إدارة الموظفين', 
    ...
    <div className="w-72 bg-[#0F172A] min-h-screen text-white p-6 flex flex-col fixed right-0 top-0 border-l border-white/5 z-50">
          ...
          return (
            <Link 
              ...
              className={`
                ...
                ${isActive 
                  ? 'bg-[#10B981] text-white' // تم تغيير لون النشط ليكون أوضح
      ...
      <div className="mt-auto border-t border-white/5 pt-6 space-y-2">
        <Link 
          ...
          className={`flex items-center gap-4 p-4 rounded-2xl transition-all ${pathname === '/dashboard/settings' ? 'bg-[#10B981] text-white' : 'text-gray-400 hover:bg-white/5'}`}
        >
          <Settings size={22} />
